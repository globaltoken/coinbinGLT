...
(function () {
	...
	coinjs.pub = 0x26;
	coinjs.priv = 0xA6;
	coinjs.multisig = 0x8D;
	coinjs.hdkey = {'prv':0x0488ade4, 'pub':0x0488b21e};
	coinjs.bech32 = {'charset':'qpzry9x8gf2tvdw0s3jn54khce6mua7l', 'version':0, 'hrp':'gt'};

	coinjs.compressed = false;

	/* other vars */
	coinjs.developer = 'yx3SsiKBoNoULoTa3TJx5MSnoA6KNBXdwB'; // globaltoken

	/* bit(coinb.in) api vars */
	coinjs.hostname	= ((document.location.hostname.split(".")[(document.location.hostname.split(".")).length-1]) == 'onion') ? 'explorer.globaltoken.org' : 'explorer.globaltoken.org';
	...
	*/
	...
	coinjs.hd = function(data){
		...
		r.master = function(pass) {
			...
			var I = hasher.getHMAC("Globaltoken seed", "TEXT", "SHA-512", "HEX");
	...
	coinjs.transaction = function() {
		...
		r.transactionHash = function(index, sigHashType) {
			...
			if((clone.ins) && clone.ins[index]){
				...
				/* SIGHASH : For more info on sig hashs see https://en.globaltoken.it/wiki/OP_CHECKSIG
					and https://globaltoken.org/en/developer-guide#signature-hash-type */
